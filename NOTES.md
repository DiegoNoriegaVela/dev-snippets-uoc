...
if [ -n "$ENC_FILE_INPUT" ] && [ -f "$ENC_FILE_INPUT" ]; then
    ENC_FILE="$ENC_FILE_INPUT"
    CANTIDAD=$(wc -l < "$ENC_FILE" | tr -d ' ')
    echo "[1/3] Usando archivo temporal existente: ${ENC_FILE}"
    echo "  Registros encontrados: ${CANTIDAD}"
else
