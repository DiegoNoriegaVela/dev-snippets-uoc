...
/* Usa time() para medir tiempo real transcurrido.    */
/* No se ve afectado por senales que alteren sleep(). */
...
static void idle_con_progreso(int intervalo, int ciclo)
{
    ...
    /* Mostrar al inicio */
    sprintf(logbuf, "Ciclo %d/%d - faltan %ds...", ciclo, MAX_CICLOS, restante);
    log_msg("WAIT  ", logbuf);

    while (1)
    {
        /* Medir tiempo real transcurrido, no iteraciones */
        time(&t_ahora);
        transcurrido = (int)difftime(t_ahora, t_inicio);
        restante     = intervalo - transcurrido;

        /* Salir cuando se cumple el intervalo real */
        if (restante <= 0)
            break;

        /* Reportar cada 60s si cambia */
        if (restante != ultimo_report && restante % 60 == 0) {
            ultimo_report = restante;
        ...
        /*
         * sleep(1) puede ser interrumpido por senales del framework ESP
         * y durar mas de 1 segundo. Eso no importa porque la condicion
         * de salida depende de time() y no del contador de iteraciones.
         */
        sleep(1);
    }
}
